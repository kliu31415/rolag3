...
const KEY_CHARGED_COLOR: Color = Color::new(1.7, 1.7, 0.0, 0.8);
...
impl RoomObject for KeyTile {
    ...
    fn draw(&mut self, ctx: &mut DrawContext) {
        ...
        let key_bow_filled = ctx.do_annular_sector(KEY_CHARGED_COLOR, &KEY_BOW_CENTER + translate, KEY_BOW_INNER_R, KEY_BOW_OUTER_R, filled_range);
        let key_bow_unfilled = ctx.do_annular_sector(KEY_UNCHARGED_COLOR, &KEY_BOW_CENTER + translate, KEY_BOW_INNER_R, KEY_BOW_OUTER_R, unfilled_range);
        
        let key_nonbow_color = if fill_frac == 1.0 {KEY_CHARGED_COLOR} else {KEY_UNCHARGED_COLOR};
...
pub fn new_key_tile(ctx: &mut NewRoomObjectContext, x: u32, y: u32) -> KeyTile {
    ...
    KeyTile { md, ro_ref, charge_amount: 0.0}
}