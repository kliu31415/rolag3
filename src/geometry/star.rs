...
pub fn get_blunt_star(num_tips: usize, inner_radius: f32, outer_radius: f32, angle: f32) -> Box<[Point]> {
    ...
    let theta = 0.5 * (num_tips - 2) as f32 * std::f32::consts::PI / (num_tips as f32) + std::f32::consts::FRAC_PI_2;
    ...
    assert!(t > 0.0);
    let side_angle = std::f32::consts::PI / (num_tips as f32);
    let v2 = &v1 + Vector::new(t * vector.x, t * vector.y);
    let v3 = &v2 + Vector::new(inner_radius as f32 * (f32::cos(side_angle) - side_angle), inner_radius as f32 * f32::sin(side_angle));
