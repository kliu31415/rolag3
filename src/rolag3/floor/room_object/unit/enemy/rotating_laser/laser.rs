use crate::{rolag3::floor::{room_object::{room_object_def::{NewRoomObjectContext, Team, Act1Response}, unit::standard_unit1::{StandardUnit1, StandardUnit1Builder, StandardUnit1BuilderReq, SuAct1Context, SuDrawContext, RofizObjType}, damage::DamageColor}, draw::{Color, DrawContext}, rofiz::rofiz_object::{Transformation, RofizObjectMovement}}, geometry::shape::{Shape, Rect, Point}};
...
pub fn new_rotating_laser(
    ...
    StandardUnit1Builder::new(StandardUnit1BuilderReq {
        ...
        .damageable(false)
        .add_secondary_hitbox(xform, rect, RofizObjType::BasicProjectile)
        .build(ctx)
}
...
fn act1(ctx: &mut SuAct1Context) -> Act1Response {
    ...
    assert!(ctx.su_ctx.secondary_ro_refs.len() == 1);
    ctx.act1_ctx.get_rofiz().move_object(&ctx.su_ctx.secondary_ro_refs[0], RofizObjectMovement::Move(move_xform));
    Act1Response::new()
}

fn draw(ctx: &mut SuDrawContext) {
    let us_data = ctx.su_ctx.us_data.downcast_mut::<RotatingLaserBasic>().unwrap();
    let xform = ctx.draw_ctx.get_rofiz().get_movable_object_xform(&ctx.su_ctx.secondary_ro_refs[0]);
