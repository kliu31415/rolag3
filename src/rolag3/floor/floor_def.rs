...
impl Floor {
            ...
            GenFloorRoomReqInfo { 
                ...
                funcs: vec![GenFloorRoomFn {
                    ...
                    func: Box::new(|ctx: &mut GenFloorRoomContext| (get_gen_room_fn_boss_generic_rect(50, 50, 
                        Box::new(|ctx: &mut NewRoomObjectContext| {
                            Box::new([
                                Rc::new(RefCell::new(new_square_rgb_star8(ctx, DamageColor::Red, 20.0, 25.0))),
                                Rc::new(RefCell::new(new_square_rgb_star8(ctx, DamageColor::Green, 25.0, 25.0))),
                                Rc::new(RefCell::new(new_square_rgb_star8(ctx, DamageColor::Blue, 30.0, 25.0))),
                            ])
                        }))(ctx))),
                },]
            },
    ...
    pub fn new_test4(renderer: &mut dyn Renderer, rng: &mut Prng, player: Rc<RefCell<Player>>) -> Self {
        ...
        let gen_normal_room_fns = vec![
            ...
            GenFloorRoomFn {weight: 2.0, func: get_gen_room_fn_common1001(4, 16, 16, 5, 20)},
        ];
        let ground_theme = GroundTheme::Monocolor(Color::new(0.02, 0.0, 0.0, 1.0));
        let wall_theme = WallTheme::Monocolor(Color::new(0.1, 0.2, 0.3, 1.0));
        let gf_args = GenFloorArgs {
